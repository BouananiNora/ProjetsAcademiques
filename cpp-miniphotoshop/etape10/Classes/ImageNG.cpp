...
{
    if (dimension.getLargeur() != img.getDimension().getLargeur() || dimension.getHauteur() != img.getDimension().getHauteur())
    {
...
//------------------constructeur------------------
    ...
    ImageNG::ImageNG()    :Image()                //par defaut
    {
      #ifdef DEBUG
  ...
  ImageNG::ImageNG(int i, const string n) : Image(i,n)  //initialisation
  {
    #ifdef DEBUG
  ...
  ImageNG::ImageNG(int i, const string n, const Dimension& dim)  : Image(i,n,dim) //initialisation avec dimension
  {
    #ifdef DEBUG
  ...
  ImageNG::ImageNG(const ImageNG& img)  : Image(img)      //copie
  {
    #ifdef DEBUG
  ...
  ImageNG::ImageNG(const string &fichier ) : Image()           //initialisation fichier
    {
      #ifdef DEBUG
          cout<<"(ImageNG)  Constructeur importation fichier "<<endl;
      #endif
...
//------------------methodes------------------
  ...
  void ImageNG::Save(ofstream& fichier) const 
  {
      ...
      float contraste = getContraste();
      fichier.write((char*)&luminance, sizeof(int));
      fichier.write((char*)&contraste, sizeof(float));
  ...
  void ImageNG::Load(ifstream& fichier) 
  {
      ...
      fichier.read((char*)&luminance, sizeof(int));
      fichier.read((char*)&contraste, sizeof(float));
...
//------------------setter------------------
  ...
  void ImageNG::setPixel(const int x, const int y , int val)
  {
    if ( (x >= dimension.getLargeur() ) || (x<0) && (y >= dimension.getHauteur()) || (y<0)) {
        throw XYException('d',"Dimension invalide" );
    }
    ...
    if (val < 0 || val > 255) 
    {
        throw RGBException(val, "erreur niveau de gris  ");
    }
...
//------------------surcharge------------------
  ...
  {
      s << "id:" << i.id << "/nom:" << i.nom << "/dimension:" << i.dimension << "/luminance:" << i.getLuminance() << "/contraste:" << i.getContraste();
      return s;
  }
